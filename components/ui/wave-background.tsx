...
export function Waves({
    ...
    const hexToRgb = (hex: string) => {
        ...
        const valid = /^[\da-fA-F]{6}$/.test(clean)
    ...
    const getLineStrokeColor = (lineIndex: number, totalLines: number) => {
        if (!lineColors || lineColors.length < 2) {
            return strokeColor
        }
        ...
        const endIndex = Math.min(lineColors.length - 1, startIndex + 1)
        const t = scaled - startIndex

        const start = hexToRgb(lineColors[startIndex] ?? "")
        const end = hexToRgb(lineColors[endIndex] ?? "")
    ...
    const setLines = () => {
        ...
        for (let i = 0; i < totalLines; i++) {
            ...
            path.setAttribute('stroke', getLineStrokeColor(i, totalLines))
